## TEnmo: 
A RESTful API server and command-line application to send and receive "TE Bucks" between users.
...
![Database schema](./img/original_tenmo_erd.png)
...
After successful authentication, an instance of `AuthenticatedUser` is stored in the `currentUser` member variable of `App`. The user's authorization token (JWT) can be accessed from `App` as `currentUser.getToken()`.

Use cases that refer to an "authenticated user": this indicates a request that includes the token as a header. You can also reference other information about the current user by using the `User` object retrieved from `currentUser.getUser()`.